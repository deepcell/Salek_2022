...
python plot_roc.py --run_ids_file runs.txt --predictions_folder preds_parquets/ --labels_file labels.parquet \
  --pos_classes NSCLC LIVER_CARCINOMA --pos_labels NSCLC --neg_labels WBC
"""
...
def get_label(label: str, sample_type: str):
    ...
    if sample_type and sample_type == "ADULT_BLOOD":
        label = "WBC"
    return label
...
def process_metrics(
    ...
    merged_df[["FP", "TP", "FN", "TN" ]] = merged_df.apply(lambda row: calculate_metrics(row, thresh, pos_labels, neg_labels), axis=1, result_type="expand")
...
def compute_roc(
    ...
    predictions = predictions.set_index(["run_id", "cell_id"])

    logging.info("Reading Labels")
    labels = read_labels(run_ids, labels_file)
    labels = labels.set_index(["run_id", "cell_id"])
    labels = labels.drop(columns=["predictions"])

    logging.info("Merging Predictions & Labels")
    merged_df = predictions.join(labels, on=["run_id", "cell_id"], how="left").reset_index()
    merged_df["label"] = merged_df.apply(lambda row: get_label(row["label"], row["sample_type"]), axis=1)
    merged_df = merged_df.dropna(subset=["label"])
    merged_df = merged_df[merged_df["label"].isin(pos_labels + neg_labels)]

    logging.info("Processing Metrics")

    metrics = {"threshold": [], "fpr": [], "recall": [] } 
    for thresh in tqdm.tqdm(thresholds, unit="threshold"):
        threshold, fpr, recall = process_metrics(merged_df, thresh, pos_labels, neg_labels)
        metrics["threshold"].append(threshold)
        metrics["fpr"].append(fpr)
        metrics["recall"].append(recall)
    roc_df = pd.DataFrame(metrics)
